...
public class WorldBuilder {
	...
	private void loopFor1(Tile[][][] tiles2) {
		for (int x = 0; x < width; x++) {
			for (int y = 0; y < height; y++) {
				for (int z = 0; z < depth; z++) {
					int floors = 0;
	...
	private int fillRegion(int region, int x, int y, int z) {
		...
		while (!open.isEmpty()){
			...
			size = loopFor2(region, size, open, p);
		}
		return size;
	}

	private int loopFor2(int region, int size, ArrayList<Point> open, Point p) {
	...
	private void connectRegionsDown(int z){
		...
		for (int x = 0; x < width; x++){
			loopFor2(z, connected, x);
		}
	}

	private void loopFor2(int z, List<Integer> connected, int x) {
		for (int y = 0; y < height; y++){
